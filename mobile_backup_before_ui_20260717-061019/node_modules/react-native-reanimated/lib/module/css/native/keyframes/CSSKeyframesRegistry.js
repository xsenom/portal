...
class CSSKeyframesRegistry {
  ...
  get(nameOrCssText) {
    ...
    const animationName = this.cssTextToNames_.get(nameOrCssText)?.values().next().value;
    if (animationName) {
      return this.nameToKeyframes_.get(animationName)?.keyframesRule;
    }
  }
  add(keyframesRule, viewTag, compoundComponentName) {
    const existingKeyframesEntry = this.nameToKeyframes_.get(keyframesRule.name);
    const existingComponentEntry = existingKeyframesEntry?.usedBy[compoundComponentName];
    ...
    if (existingKeyframesEntry) {
      ...
      this.nameToKeyframes_.set(keyframesRule.name, {
        keyframesRule,
        usedBy: {
          [compoundComponentName]: new Set([viewTag])
        }
      });
    }
    ...
    const normalizedKeyframesConfig = keyframesRule.getNormalizedKeyframesConfig(compoundComponentName);
    registerCSSKeyframes(keyframesRule.name, compoundComponentName, normalizedKeyframesConfig);
  }
  remove(animationName, viewTag, compoundComponentName) {
    ...
    if (Object.keys(keyframesEntry.usedBy).length === 0) {
      ...
      const namesForCssText = this.cssTextToNames_.get(keyframesEntry.keyframesRule.cssText);
      if (namesForCssText) {
