...
export default class CSSAnimationsManager {
  attachedAnimations = [];
  constructor(shadowNodeWrapper, viewTag, compoundComponentName) {
  ...
  update(animationProperties) {
    ...
    if (animationUpdates) {
      ...
      applyCSSAnimations(this.shadowNodeWrapper, this.compoundComponentName, animationUpdates);
    }
  }
  unmountCleanup() {
  ...
  registerKeyframesUsage(processedAnimations) {
    ...
    processedAnimations.forEach(({
      ...
      cssKeyframesRegistry.add(keyframesRule, this.viewTag, this.compoundComponentName);
    ...
    this.attachedAnimations.forEach(({
      ...
      if (!newAnimationNames.has(name)) {
        cssKeyframesRegistry.remove(name, this.viewTag, this.compoundComponentName);
      }
    });
  }
