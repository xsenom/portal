...
import { getMemorySafeCapturableConsole, setupConsole, setupConsoleForwarding, setupSerializer } from './initializers/initializers';
...
export function createWorkletRuntime(nameOrConfig, initializer) {
  const runtimeBoundCapturableConsole = globalThis._WORKLETS_BUNDLE_MODE_ENABLED && !__DEV__ ? null : getMemorySafeCapturableConsole();
  let name;
  let initializerFn;
  let queue = 'default';
  ...
  const customQueue = typeof queue === 'object' && queue !== null ? queue : undefined;
  if (initializerFn && !isWorkletFunction(initializerFn)) {
    throw new Error('[Worklets] The initializer passed to `createWorkletRuntime` is not a worklet.');
  }
...
export function scheduleOnRuntime(workletRuntime, worklet, ...args) {
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `scheduleOnRuntime` is not a worklet.');
  }
...
export function scheduleOnRuntimeWithId(runtimeId, worklet, ...args) {
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `scheduleOnRuntimeWithId` is not a worklet.');
  }
...
export function runOnRuntime(workletRuntime, worklet) {
  ...
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `runOnRuntime` is not a worklet.');
  }
...
export function runOnRuntimeSync(workletRuntime, worklet, ...args) {
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `runOnRuntimeSync` is not a worklet.');
  }
...
export function runOnRuntimeSyncWithId(runtimeId, worklet, ...args) {
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `runOnRuntimeSyncWithId` is not a worklet.');
  }
...
export function runOnRuntimeAsync(workletRuntime, worklet, ...args) {
  if (__DEV__) {
    if (!isWorkletFunction(worklet)) {
      throw new Error('[Worklets] The function passed to `runOnRuntimeAsync` is not a worklet.');
    }
  }
...
export function runOnRuntimeAsyncWithId(runtimeId, worklet, ...args) {
  if (__DEV__ && !isWorkletFunction(worklet)) {
    throw new Error('[Worklets] The function passed to `runOnRuntimeAsyncWithId` is not a worklet.');
  }
  return new Promise((resolve, reject) => {
    if (__DEV__) {
      createSerializable(worklet);
